...
void viewport(int x, int y){
	glViewport(x*funcoes.viewportW, y*funcoes.viewportH, funcoes.viewportW, funcoes.viewportH);
		...
		for(unsigned int i = 0; i<funcoes.get_nucleos()->size(); i++){
				...
				glBegin(GL_LINE_STRIP);
					for(unsigned int j = 0; j < funcoes.get_nucleos()->at(i)->size(); j++){
						glVertex2i(650-funcoes.get_nucleos()->at(i)->at(j)->get_x(), 650-funcoes.get_nucleos()->at(i)->at(j)->get_y());
					}
				glEnd();
		}
}
...
void display() {
	...
	for(int i = 0; i<funcoes.qtd_dinosH; i++){
		for(int j = 0; j<funcoes.qtd_dinosV; j++){
			viewport(i,j);
		}
	}
...
void reshape(int x, int y){
	funcoes.qtd_dinosH = x/funcoes.viewportW+1;
	funcoes.qtd_dinosV = y/funcoes.viewportH+1;
}
...
int main(int argc, char** argv) {
	...
	glutCreateWindow("Atividade 5.4"); // Create a window with the given title
